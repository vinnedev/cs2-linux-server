...
| Gerenciador      | Bash scripts + Docker Compose                           |
...
mindmap
  root((CS2 Server))
    ...
    Automação
      install.sh (SteamCMD)
      check-updates.sh (mods)
      add-map.py (workshop)
      patch-gameinfo.sh (metamod hook)
```
...
- **`.env` como fonte única**: `start.sh` lê o arquivo e exporta variáveis; mesmas chaves usadas pelo `docker-compose.yml`.
